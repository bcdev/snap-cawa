...
class cloud_core:
    def __init__(self,corefile=os.path.join(about_me()['cawadir'],'luts','cloud_complete_core_meris.nc4'),used_ab=None):
        ...
        def jforward(woo,geo):
            ...
            inp[:4],inp[4:]=woo,geo            
            if self.ab_idx is None:
                return self._jacobian(inp)[:,:4]
            else:
                return self._jacobian(inp)[(0,1+self.ab_idx),:4]
        
        self.jforward=jforward
 
        #min_state
        a=np.array([self.axes[i].min() for i in range(4)])
        #max_state
        b=np.array([self.axes[i].max() for i in range(4)])       
    ...
    def _do_inversion(self,data,sa=SA,se=None,xa=XA):
    ...
    def estimator(self,inp,sa=SA,se=None,xa=XA):
        if se is None: se=self.SE
        #data=copy.deepcopy(inp)
        data=inp
        self._do_inversion(data,sa=sa,se=se,xa=xa)
...
if __name__=='__main__':
    ...
    #inp={'suz':10.,'vie':40.,'azi':170.,'alb':0.1,'dwl':-0.2
        #,'rtoa':{'12':0.188,'13':0.10,'14':0.13,'15':0.18}}
    #print cc.estimator(inp)['res'].x
    cc=cloud_complete_core.cloud_core('luts/cloud_complete_core_olci.nc4')
    ccc=cloud_complete_core.cloud_core('luts/cloud_complete_core_olci.nc4',used_ab='14')
    print cc.forward([200,1.2,0.8,0.3],[0.1,20.,30.,170,0.2])
    print ccc.forward([200,1.2,0.8,0.3],[0.1,20.,30.,170,0.2])
    print cc.jforward([200,1.2,0.8,0.3],[0.1,20.,30.,170,0.2])
    print ccc.jforward([200,1.2,0.8,0.3],[0.1,20.,30.,170,0.2])
    inp={'suz':10.,'vie':40.,'azi':170.,'alb':0.1,'dwl':-0.2
        ,'rtoa':{'12':0.188,'13':0.10,'14':0.13,'15':0.18}}
    print cc.estimator(inp)['res'].x
    inp={'suz':10.,'vie':40.,'azi':170.,'alb':0.1,'dwl':-0.2
