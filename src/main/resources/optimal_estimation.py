...
def numerical_jacoby(a, b, x, fnc, nx, ny, delta):
    ...
    dx=(b - a) * delta
    jac = np.empty((ny, nx),order='F')  # zeilen zuerst, spalten später!!!!!!!
    for ix in range(nx):
        dxm = x * 1.
        dxp = x * 1.
        dxm[ix] = dxm[ix] - dx[ix]
...
def numerical_jacoby_minus(a, b, x, fnc, nx, ny, delta):
    '''
...
def my_optimizer(
   ...
   jparams: are directly passed to jaco (e.g. geometry, special temperatures, profiles aerosols ...)
    ...
    if method == 0:
        # Newton Step
        def operator(x,y,k):
            return oec.newton_operator(a,b,x,y,k)
        def reterrcov(k):             
            return np.zeros((k.shape[1],k.shape[1]))
        def diagnose(x, y, k,sr):
            return oec.newton_gain_aver_cost(x, y, k)
    elif method == 1:
        # Newton with measurement error
        def operator(x,y,k):
            return oec.newton_operator_with_se(a, b, x, y, k, sei)
        def reterrcov(k):             
            return oec.newton_se_ret_err_cov(k,sei)
        def diagnose(x, y, k, sr):
            return oec.newton_se_gain_aver_cost(x, y, k, sei, sr)
    elif method == 2:
        # Optimal Estimation
        def operator(x,y,k):
    ...
    def func_inverse(
        ...
        if check_input:
            for ele in ('a','b','yyy','xa','fg','sei','sai'):
                print ele,eval('type(%s)'%ele), 
                print eval('%s.flags'%ele),
...
def test():
    ...
    A = np.array([0.1, 0.1, 0.1],order='F')
    # upper bound of state
    B = np.array([10., 10., 10.],order='F')
    AA = {'a': A[0:2], 'b': A[0:2], 'c': A}
    BB = {'a': B[0:2], 'b': B[0:2], 'c': B}

    # SE measurement error covariance
    SEa = np.array([[10., 0., 0.], [0., 10., 0.], [0., 0., 100.]],order='F') * .1
    SEb = np.array([[10., 0., 0.], [0., 10., 0.], [0., 0., 10.]],order='F') * 10.
    SEc = np.array([[10., 0.], [0., 10.]],order='F') * 0.1
    SE = {'a': SEa, 'b': SEb, 'c': SEc}

    # SA apriori error covariance
    SAa = np.array([[1., 0.], [0., 1.]],order='F') * 1.
    SAb = np.array([[1., 0.], [0., 1.]],order='F') * 1.
    SAc = np.array([[1., 0., 0.], [0., 1., 0.], [0., 0., 1.]],order='F') * 100.
    SA = {'a': SAa, 'b': SAb, 'c': SAc}

    # XA prior knowledge
    XA = {'a': np.array([3.7, 5.6],order='F'),
          'b': np.array([3.7, 5.6],order='F'),
          'c': np.array([3.7, 5.6, 8.5],order='F')
          }

    # XT to test
    XT = {'a': np.array([3.5, 6.5],order='F'),
          'b': np.array([3.5, 6.5],order='F'),
          'c': np.array([3.5, 6.5, 5.8],order='F')
          }
